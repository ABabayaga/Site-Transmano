  ...
  return (
    <header className="relative left-1/2 w-screen -translate-x-1/2 text-left font-[family-name:var(--sans)]">
      <div className="bg-[#131313] text-white/85">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-6 px-8 py-2.5 text-[13px] max-[860px]:px-5 max-[860px]:py-2">
      ...
      <nav className="mx-auto flex max-w-[1440px] flex-wrap items-center justify-between gap-6 border-b border-[var(--border)] bg-[var(--bg)] px-8 py-4.5 max-[860px]:px-5 max-[860px]:py-4">
        <ul className="m-0 flex flex-wrap items-center gap-7 p-0">
          {navLinks.map((label) => (
            <li key={label} className="list-none">
              <a
                href={`#${label.toLowerCase().replace(/\s+/g, '-')}`}
                className="text-[13.5px] font-bold tracking-[0.3px] text-[var(--text-h)] uppercase no-underline transition-colors hover:text-[#e0b30f]"
              >
                {label}
              </a>
            </li>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-4">
          <button
            type="button"
            className="flex items-center gap-2 rounded-full border border-[var(--border)] px-5 py-2.5 text-[12.5px] font-bold tracking-[0.3px] text-[var(--text-h)] uppercase transition-colors hover:border-[var(--text-h)]"
          >
