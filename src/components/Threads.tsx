...
interface ThreadsProps {
  ...
  /** Second colour; threads fan from `color` to `colorB` across the bundle. */
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Accumulate each thread's own colour so the bundle can span two brand hues
    // in a single pass, instead of stacking a second full shader layer.
    vec3 accum = vec3(0.0);
    float accum_w = 0.0;
    for (int i = 0; i < u_line_count; i++) {
        ...
        line_strength *= (1.0 - l);
        // lineFn fades out high p, so spread the two colours across the range
        // that actually renders rather than the full 0..1 sweep.
        accum += mix(uColor, uColorB, smoothstep(0.0, 0.22, p)) * l;
        accum_w += l;
    }

    float colorVal = 1.0 - line_strength;
    vec3 tint = accum_w > 0.0001 ? accum / accum_w : uColor;
    fragColor = vec4(tint * colorVal, colorVal);
}
...
const Threads: React.FC<ThreadsProps> = ({
  ...
  const propsRef = useRef({ color, colorB, amplitude, distance, enableMouseInteraction });
  useEffect(() => {
    propsRef.current = { color, colorB, amplitude, distance, enableMouseInteraction };
  });
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColorB: { value: new Color(...(propsRef.current.colorB ?? propsRef.current.color)) },
    ...
    // Reduced motion: paint the threads once and hold that frame.
    ...
    function update(t: number) {
      ...
      if (motionQuery.matches) {
        if (staticFrameDrawn) return;
        staticFrameDrawn = true;
      } else {
        staticFrameDrawn = false;
      }

      const { color, colorB, amplitude, distance, enableMouseInteraction } = propsRef.current;

      program.uniforms.uColor.value.set(...color);
      program.uniforms.uColorB.value.set(...(colorB ?? color));
      ...
      program.uniforms.iTime.value = motionQuery.matches ? 0 : t * 0.001;
